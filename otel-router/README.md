...
endpoint and fans it out to two destinations: your SIEM and your application
...
[OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) (core
distribution, ~40MB) with a single config file: no custom code to maintain,
and production-grade batching, retries and queueing for free.

```
                        ┌──────────────┐
 your services ──OTLP──▶│  otel-router │──OTLP──▶ SIEM
 (one endpoint)         │  :4317/:4318 │──OTLP──▶ app backend
                        └──────────────┘
```
...
`sink-siem` and `sink-app`. That is the fan-out working. Ctrl-C to stop.

## Running against real destinations

Build and run the router with your endpoints and auth supplied as environment
...
Then point your services' OTLP exporters at the router:
...
If a destination later needs a vendor-specific format (Splunk HEC, Elastic,
etc.), switch the base image in the `Dockerfile` to
`otel/opentelemetry-collector-contrib` (same version tag) and swap that
exporter block for the vendor one. Nothing else changes.
...
- Each exporter has an in-memory sending queue with retry and backoff; brief
  ...
  router restart. If you need durability, add a
  [`file_storage`](https://github.com/open-telemetry/opentelemetry-collector-extensions)
  extension to the sending queues.

## Layout

```
Dockerfile               pinned Collector image + config
config/otel-router.yaml  the router: one source, two destinations
